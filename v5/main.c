...
// версия без блокировок с локальным хранением вектора сил в каждом потоке
...
int main(int argc, char* argv[])
{
    ...
    struct particle* f[omp_get_max_threads()];   // Сила, действующая на каждую частицу
    ...
    double dt = 1e-5;
#pragma omp parallel num_threads(8) // Параллельный регион активируется один раз
    {
